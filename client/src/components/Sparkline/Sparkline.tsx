...
const DOT_RADIUS = 4;
const TOOLTIP_MIN_PCT = 14;
const TOOLTIP_MAX_PCT = 86;
...
function Tooltip({ geom, h }: { geom: Geometry; h: number }) {
  const leftPct = Math.min(TOOLTIP_MAX_PCT, Math.max(TOOLTIP_MIN_PCT, (geom.xs[h] / W) * 100));
  return (
...
export function Sparkline({ points }: { points: HistoryPoint[] }) {
  ...
  const geom = useMemo(() => buildGeometry(points), [points]);
  ...
  return (
    <div className="sparkline" ref={wrapRef} onPointerMove={onPointerMove} onPointerLeave={() => setHover(null)}>
      ...
      >
        ...
        {h != null && <line className="crosshair" x1={geom.xs[h]} x2={geom.xs[h]} y1={PAD.top} y2={H - PAD.bottom} />}
